\section*{Problem 21}
    ...
    \begin{tikzpicture}[overlay, remember picture, >=stealth, nodes={align=left, inner ysep=2pt}, <-]
        \path (Kepler.south) ++ (0,-0.8em) node[anchor=north west, color=gray!67] (Kepler node){aaaa};
        \draw [color=gray!57](Kepler.south) |- ([xshift=+0.3ex, color=gray]Kepler node.south east);
    \end{tikzpicture}