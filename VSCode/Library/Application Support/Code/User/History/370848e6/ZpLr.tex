\begin{titlepage}
	\begin{tikzpicture}[remember picture, overlay, pencildraw/.style={
		...
		{ \fontsize{45}{45}\CJKfamily{coverfont}\texttt{spBook}};

		\node[anchor=center] (volume) at ([yshift=-7em] title.center) {\fontsize{30}{30}\CJKfamily{coverfont}\textbf{DEMO}};

		\node[anchor=west] (author) at ([yshift=-5em, xshift=-4pt] volume.south west) {\fontsize{18}{18}\CJKfamily{cover-author-font}Sweet Pastry};
		\node[anchor=west] at ([xshift=2.4em, yshift=-20pt] author.east) {\fontsize{18}{18}\CJKfamily{cover-author-font}yshift=-20pt};
		\node[anchor=west] at ([xshift=2.4em, ] author.east) {Coverpage code \textbf{heavily} based on \href{https://github.com/wenweili/AlJabr-1/blob/master/coverpage.tex}{Li Wen-wei} and \href{https://texample.net/parallelepiped/}{Axel Pavillet}}
